...
# Define the setups
setups <- list(
  list(label = "m=1", m_func = function(N) {
    1
  }),
  list(label = "m=round(N/10)", m_func = function(N) {
    max(1, floor(N / 10))
  }),
  list(label = "m=N", m_func = function(N) {
    N
  })
)
...
for (setup_idx in 1:length(setups)) {
  ...
  for (N_idx in 1:length(N_values)) {
    ...
    for (sim in 1:num_simulations) {
      ...
      # Estimate the signal privately
      estimated_signal <- federated_estimator(ns, pb, s_point,
        Y = Y, X = X, grid_size = grid_size,
        max_level = max_level, wavelet_family = wavelet_family,
        boundary = boundary, S = S, tau = tau, c_psi = c_psi
      )
...
ggsave(filename = file.path(script_dir, "average_total_MSE_vs_N_log_filtered.pdf"), width = 8, height = 6)
...
# Save the plot as a PDF
ggsave(filename = file.path(script_dir, "average_point_MSE_vs_N_log_filtered.pdf"), width = 8, height = 6)
