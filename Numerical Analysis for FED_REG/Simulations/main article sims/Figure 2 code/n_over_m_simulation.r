...
n_values <- 2^(0:floor(log2(N))) # This gives 1, 2, 4, ..., up to the largest power of 2 <U+2264> N
...
results_file <- file.path(script_dir, "n_over_m_results_log.RData")
save(n_values, average_total_MSEs, average_point_MSEs, file = results_file)
...
if (plots) {
  ...
  plot(nm_ratios, average_total_MSEs,
  ...
  plot(nm_ratios, average_point_MSEs,
    log = "xy", type = "b", pch = 19, col = "red",
...
if (plots) {
  ...
  plot_data_long <- plot_data %>%
    pivot_longer(
      cols = c(Average_Total_MSE, Average_Point_MSE),
      names_to = "Metric",
      values_to = "MSE"
    )
  ...
  ggsave(filename = file.path(script_dir, "Combined_MSE_vs_nm_ratio.pdf"), width = 8, height = 6)
}
