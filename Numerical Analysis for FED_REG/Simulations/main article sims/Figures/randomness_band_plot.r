...
source(file.path(dirname(script_dir), "Figure 1 code", "wavelet_helper_functions.r"))
...
for (idx in 1:length(eps_values)) {
  ...
  for (sim in 1:num_simulations) {
    ...
    estimated_signal <- federated_estimator(ns, pb, s,
      Y = Y, X = X, grid_size = grid_size,
...
# For PDF format
pdf(file.path(script_dir, "randomness_over_privacy_mechanism.pdf"), width = 8, height = 6)
...
legend("topright",
  legend = c("True Signal", legend_labels),
  col = c("blue", colors), lwd = 2, lty = c(1, line_types), bg = "white"
)
dev.off()

# Plot only the first curve and the scatter points
pdf(file.path(script_dir, "scatter_and_first_curves.pdf"), width = 8, height = 6)

# Plotting the results with adjusted y-limits
plot(
  x = x_values, y = signal, type = "l", col = "blue", lwd = 4,
  main = "",
  xlab = "X-values", ylab = "Y-values", ylim = ylim_range
)
...
pdf(file.path(script_dir, "Wavelet_Estimation_Privacy_Mechanism.pdf"), width = 8, height = 6)

# Plot the true signal
plot(
  x = x_values, y = signal, type = "l", col = "black", lwd = 2, lty = 1,
  xlab = "X-values", ylab = "Y-values", ylim = ylim_range
)
...
plot(
  x = x_values, y = signal, type = "l", col = "black", lwd = 3, lty = 1, # True signal is thicker and solid
  xlab = "X-values", ylab = "Y-values", ylim = ylim_range
)

# Add scatter points (scaled X), smaller and lighter
points(
  x = X / grid_size, y = Y, col = adjustcolor("black", alpha.f = 0.3),
  pch = 16, cex = 0.5
) # Smaller points with lighter color
...
legend("topright",
  legend = c("True Signal", legend_labels),
  col = "black", lwd = c(3, 2, 2, 2, 2), lty = c(1, line_types), bty = "n"
) # True signal thicker
...
ggplot() +
  ...
  geom_point(
    data = first_estimates_df_subsampled,
  ...
  guides(
    color = guide_legend(override.aes = list(shape = c(NA, shape_types[-1]))),
...
ggsave(filename = file.path(script_dir, "first_curves.pdf"), width = 8, height = 6)
...
# Save the plot as a PDF
ggsave(filename = file.path(script_dir, "Bands.pdf"), width = 8, height = 6)
