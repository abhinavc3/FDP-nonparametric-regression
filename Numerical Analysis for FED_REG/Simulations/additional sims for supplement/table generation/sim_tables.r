...
for (eps in epsilon_values) {
  ...
  for (i in seq_along(machine_counts)) {
    ...
    for (sim in 1:num_simulations) {
      ...
      # Estimate signal
      estimated_signal <- federated_estimator(
        ns, pb, s_point, Y, X, grid_size, max_level,
        wavelet_family, boundary, S, tau, c_psi
      )
...
#' Create and save a LaTeX table from simulation results.
#'
#' @param results_list Named list of simulation summaries to tabulate.
#' @param caption_text Caption text for the LaTeX table.
#' @param label_text Label text for the LaTeX table.
#' @param file_name Output file name written in the tables directory.
#' @return Invisibly writes a LaTeX table to disk and returns `NULL`.
create_latex_table <- function(results_list, caption_text, label_text, file_name) {
  ...
  table_code <- capture.output(print(latex_table,
    include.rownames = FALSE, sanitize.text.function = identity,
    booktabs = TRUE, table.placement = "ht", hline.after = NULL,
    add.to.row = list(
      pos = list(-1, 0, nrow(results_df)),
      command = c("\\toprule\n", "\\midrule\n", "\\bottomrule\n")
    )
  ))

  # Write the LaTeX code to the specified file
  writeLines(table_code, con = file.path(tables_dir, file_name))

  # Optionally, print a message indicating that the file has been saved
  cat("LaTeX table saved to", file.path(tables_dir, file_name), "\n")
}
