...
#' Preprocess one source for the individual heart-data wavelet analysis.
#'
#' @param ppX Numeric covariate vector.
#' @param ppY Numeric response vector.
#' @param grid_size Number of grid points used for wavelet reconstruction.
#' @return A list containing sorted grid indices, standardized responses, plotting grid values, and scaling parameters.
...
#' Estimate private and non-private wavelet signals for one preprocessed source.
#'
#' @param X_sorted Sorted integer grid locations.
#' @param Y_sorted Standardized response values aligned with `X_sorted`.
#' @param grid_size Number of grid points used for wavelet reconstruction.
...
#' @param L_max_np Non-private maximum resolution level.
#' @param eps_values Named vector of privacy budgets to evaluate.
#' @return A named list of reconstructed signals on the standardized scale.
...
#' Run the full individual-source wavelet analysis pipeline.
#'
#' @param ppX Numeric covariate vector.
#' @param ppY Numeric response vector.
...
#' @param s Smoothness parameter used by the estimator.
#' @param eps_values Named vector of privacy budgets to evaluate.
#' @return A list containing plotting coordinates and reconstructed signals on the original response scale.
run_analysis <- function(ppX, ppY, num_bins, grid_size, wavelet_family, boundary, S, s, eps_values) {
  ...
  est_signals_std <- estimate_wavelet_signals(
    X_sorted, Y_sorted, grid_size,
...
#' Plot observed heart-data points together with fitted wavelet curves.
#'
#' @param x_values Grid values used for the fitted signals.
#' @param ppX Observed covariate values.
#' @param ppY Observed response values.
#' @param est_signals Named list of reconstructed signals.
#' @param title Plot title.
#' @return A `ggplot2` object showing the observations and fitted curves.
...
for (src in sources) {
  ...
  res <- run_analysis(
    subset_data$age, subset_data$thalach,
...
#' Compute the coefficient of determination against observed data via interpolation.
#'
#' @param est_signal Numeric vector of fitted signal values.
#' @param x_values Grid values associated with `est_signal`.
