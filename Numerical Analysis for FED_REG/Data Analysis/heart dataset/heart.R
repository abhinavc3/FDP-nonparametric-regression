...
#' Compute the F1 score for binary predictions.
#'
#' @param pred Numeric or binary predicted labels.
#' @param tru Numeric or binary true labels.
#' @return A numeric scalar F1 score.
...
#' Rescale feature columns relative to the training data range.
#'
#' @param xmat Numeric feature matrix to rescale.
#' @return A numeric matrix on the same scale used by the classification workflow.
...
#' Collapse multi-class labels into a binary outcome.
#'
#' @param y Response vector.
#' @return A numeric vector with all positive classes mapped to `1`.
twoclass <- function(y) {
  y <- c(y)
  y[y > 1] <- 1
  return(y)
...
#' Evaluate the best thresholded logistic-regression accuracy on the target source.
#'
#' @return A numeric scalar maximum accuracy over the tested thresholds.
logist_acc <- function() {
...
#' Evaluate private and non-private heart-classification methods for one privacy budget.
#'
#' @param eps1 Privacy budget supplied to the adaptive kernel methods.
#' @return A data frame containing accuracy, F1-score, and AUC summaries for each method.
heart_accuracy <- function(eps1) {
  ...
  train_source <- list(
    list(X = scaler(train_source1X), Y = twoclass(train_source1Y)),
...
#' Summarize Monte Carlo output with mean, standard deviation, and normal-approximation bounds.
#'
#' @param x Numeric vector of simulation results.
#' @return A named numeric vector containing the mean, standard deviation, and two confidence-limit summaries.
...
#' Append summary statistics to a simulation-result table.
#'
#' @param sim_result Data frame or matrix containing identifiers in the first columns and simulation draws afterward.
#' @param t Number of leading columns to keep before summarizing the simulation draws.
#' @return A combined object with the leading identifiers and summarized simulation statistics.
...
#' Define the custom `ggplot2` theme used for the heart-data summary plots.
#'
#' @return A `ggplot2` theme object.
custom_theme <- function() {
  theme_minimal() +
    theme(
      text = element_text(size = 10), # Default text size and style
