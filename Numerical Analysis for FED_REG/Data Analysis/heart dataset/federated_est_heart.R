...
#     <U+2022> When use_subsample = TRUE   <U+2192> original uniform-bin subsample
#     <U+2022> When use_subsample = FALSE  <U+2192> return the data unchanged
# ------------------------------------------------------------------
#' Optionally subsample observations to balance the age distribution across bins.
#'
#' @param ppX Numeric covariate vector.
#' @param ppY Numeric response vector aligned with `ppX`.
#' @param num_bins Number of bins used to partition the clipped age range.
#' @param draw_per_bin Optional cap on the number of sampled observations per bin.
#' @param use_subsample Logical flag indicating whether subsampling is applied.
#' @return A list with elements `ppX` and `ppY` containing the returned sample.
subsample_uniform_ppX_ppY <- function(ppX, ppY,
  ...
  if (!is.null(draw_per_bin)) {
    ...
    if (draw_per_bin > max_samples_per_bin) {
      warning(
        "draw_per_bin exceeds minimum bin count. Using ",
...
#' Preprocess one source for the federated heart-data analysis.
#'
#' @param ppX Numeric covariate vector.
#' @param ppY Numeric response vector.
#' @param grid_size Number of grid points used for wavelet reconstruction.
#' @return A list containing sorted grid indices, standardized responses, plotting grid values, and source-specific scaling parameters.
...
#' Run the preprocessing pipeline for one heart-data source.
#'
#' @param data Data frame for one source.
#' @param num_bins Number of bins used for optional subsampling.
#' @param grid_size Number of grid points used for wavelet reconstruction.
...
#' @param s Smoothness parameter used by the estimator.
#' @return A preprocessed source-specific list ready for federated estimation.
...
for (pb_val in privacy_values) {
  ...
  est_signal <- federated_estimator(ns, pb_vector, s,
    Y = Y_fed, X = X_fed, grid_size = grid_size,
...
###############################
#' Compute the coefficient of determination for observed and predicted values.
#'
#' @param predicted Numeric vector of fitted values.
#' @param observed Numeric vector of observed values.
#' @return A numeric scalar `R^2` value.
...
#' Plot federated fitted curves together with the observed heart-data points.
#'
#' @param x_values Grid values used for the fitted signals.
#' @param true_x Observed covariate values.
#' @param true_y Observed response values.
#' @param federated_estimates Named list of fitted signals by privacy level.
#' @param title Plot title.
#' @return A list containing the `ggplot2` object and the assembled estimate data frame.
plot_estimated_signal_with_true_data <- function(x_values, true_x, true_y, federated_estimates, title) {
  ...
  true_data <- data.frame(
    x = pmin(pmax(true_x, lower_age), upper_age),
    y = true_y,
...
#' Plot source-specific federated estimates for a selected machine.
#'
#' @param source_index Integer index of the source to visualize.
#' @param x_values Grid values used for the fitted signals.
#' @param preproc_list List of preprocessed source objects.
#' @param federated_estimates Named list of source-specific fitted signals by privacy level.
#' @return A `ggplot2` object for the selected source.
plot_source_estimates <- function(source_index, x_values, preproc_list, federated_estimates) {
  ...
  p <- ggplot() +
    ...
    labs(
      title = paste0("Source ", source_index, " Estimates"),
